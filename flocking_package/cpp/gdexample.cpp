...
static const int max_instances = 32768;
static const int max_flocks = 2048;

void FlockDotCpp::_bind_methods() {
    ClassDB::bind_method(D_METHOD("set_current_position"), &FlockDotCpp::set_current_position);
    ClassDB::bind_method(D_METHOD("get_current_position"), &FlockDotCpp::get_current_position);
    ClassDB::add_property("FlockDotCpp", PropertyInfo(Variant::VECTOR2, "current_position"), "set_current_position", "get_current_position");

    ClassDB::bind_method(D_METHOD("set_target_position"), &FlockDotCpp::set_target_position);
    ClassDB::bind_method(D_METHOD("get_target_position"), &FlockDotCpp::get_target_position);
    ClassDB::add_property("FlockDotCpp", PropertyInfo(Variant::VECTOR2, "target_position"), "set_target_position", "get_target_position");

    ClassDB::bind_method(D_METHOD("set_velocity"), &FlockDotCpp::set_velocity);
    ClassDB::bind_method(D_METHOD("get_velocity"), &FlockDotCpp::get_velocity);
    ClassDB::add_property("FlockDotCpp", PropertyInfo(Variant::VECTOR2, "velocity"), "set_velocity", "get_velocity");

    ClassDB::bind_method(D_METHOD("set_color"), &FlockDotCpp::set_color);
    ClassDB::bind_method(D_METHOD("get_color"), &FlockDotCpp::get_color);
    ClassDB::add_property("FlockDotCpp", PropertyInfo(Variant::COLOR, "color"), "set_color", "get_color");
}

FlockDotCpp::FlockDotCpp() {
}

FlockDotCpp::~FlockDotCpp() {
}

void FlockDotCpp::set_current_position(Vector2 _current_position) {
    current_position = _current_position;
}

Vector2 FlockDotCpp::get_current_position() const {
    return current_position;
}

void FlockDotCpp::set_target_position(Vector2 _target_position) {
    target_position = _target_position;
}

Vector2 FlockDotCpp::get_target_position() const {
    return target_position;
}

void FlockDotCpp::set_velocity(Vector2 _velocity) {
    velocity = _velocity;
}

Vector2 FlockDotCpp::get_velocity() const {
    return velocity;
}

void FlockDotCpp::set_color(Color _color) {
    color = _color;
}

Color FlockDotCpp::get_color() const {
    return color;
}

void FlockCpp::_bind_methods() {
    ClassDB::bind_method(D_METHOD("set_owner"), &FlockCpp::set_owner);
    ClassDB::bind_method(D_METHOD("get_owner"), &FlockCpp::get_owner);

    ClassDB::bind_method(D_METHOD("set_size"), &FlockCpp::set_size);
    ClassDB::bind_method(D_METHOD("get_size"), &FlockCpp::get_size);

    ClassDB::bind_method(D_METHOD("add_dot"), &FlockCpp::add_dot);
}
...
void FlockCpp::set_owner(Sprite2D* _owner) {
    owner = _owner;
}

Sprite2D* FlockCpp::get_owner() const {
    return owner;
}

void FlockCpp::set_size(Vector2 _size) {
    size = _size;
}

Vector2 FlockCpp::get_size() const {
    return size;
}

int FlockCpp::get_dots_count() {
    return dots.size();
}

void FlockCpp::add_dot(FlockDotCpp* _dot) {
    dots.push_back(_dot);
}

void GDExample::_bind_methods() {
    ClassDB::bind_method(D_METHOD("get_noise_texture"), &GDExample::get_noise_texture);
    ClassDB::bind_method(D_METHOD("set_noise_texture", "p_noise_texture"), &GDExample::set_noise_texture);
    ClassDB::add_property("GDExample", PropertyInfo(Variant::OBJECT, "noise_texture", PROPERTY_HINT_RESOURCE_TYPE, "Texture2D"), "set_noise_texture", "get_noise_texture");

    ClassDB::bind_method(D_METHOD("add_flock"), &GDExample::add_flock);
    ClassDB::bind_method(D_METHOD("remove_flock"), &GDExample::remove_flock);
}
...
static GDExample* process_instance = nullptr;
...
void GDExample::_ready() {
    ...
    Ref<RDShaderFile> shader_file = ResourceLoader::get_singleton()->load("res://shaders/flock_compute.glsl");
...
void GDExample::set_noise_texture(Ref<Texture2D> p_noise_texture) {
    noise_texture = p_noise_texture;
}

Ref<Texture2D> GDExample::get_noise_texture() const {
    return noise_texture;
}

void GDExample::add_flock(FlockCpp* flock) {
    flocks.push_back(flock);
}

void GDExample::remove_flock(Ref<FlockCpp> flock) {
    flocks.erase(flock);
}

static void process_flocks_buffer(uint32_t flock_index) {
    Ref<FlockCpp> flock = process_instance->flocks[flock_index];

    for (int dot_index = 0; dot_index < flock->dots.size(); dot_index++) {
        Ref<FlockDotCpp> dot = flock->dots[dot_index];
...
void GDExample::update_flock_array() {
    int index = 0;
    for (int flock_index = 0; flock_index < flocks.size(); flock_index++) {
        Ref<FlockCpp> flock = flocks[flock_index];

        flocks_array[index * 12] = flock->owner->get_global_position().x;
        flocks_array[index * 12 + 1] = flock->owner->get_global_position().y;

        flocks_array[index * 12 + 2] = flock->owner->get_global_scale().x;
        flocks_array[index * 12 + 3] = flock->owner->get_global_scale().y;

        flocks_array[index * 12 + 4] = flock->size.x;
        flocks_array[index * 12 + 5] = flock->size.y;

        flocks_array[index * 12 + 6] = flock->owner->get_offset().x;
        flocks_array[index * 12 + 7] = flock->owner->get_offset().y;

        flocks_array[index * 12 + 8] = flock->owner->is_flipped_h() ? 1.0f : 0.0f;
        flocks_array[index * 12 + 9] = flock->owner->is_flipped_v() ? 1.0f : 0.0f;

        flocks_array[index * 12 + 10] = flock->owner->is_centered() ? 1.0f : 0.0f;
        flocks_array[index * 12 + 11] = flock->owner->get_global_rotation();

        index++;
    }
}

void GDExample::update_buffers(double delta) {
    dots_bytes = dots_array.to_byte_array();
    flocks_bytes = flocks_array.to_byte_array();
...
void GDExample::retrieve_output() {
...
static void process_mesh_instance(uint32_t flock_index) {
    ...
    for (int dot_index = 0; dot_index < flock->dots.size(); dot_index++) {
        ...
        RenderingServer::get_singleton()->multimesh_instance_set_transform_2d(multimesh_rid, instance_index, Transform2D{ 0, Vector2{8.0f, 8.0f}, 0, dot->current_position });
        RenderingServer::get_singleton()->multimesh_instance_set_color(multimesh_rid, instance_index, dot->color);
    }
}

void GDExample::update_mesh() {
