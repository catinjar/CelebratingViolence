...
void main() {
    ...
    if (flock.flip_h != 0.0)
      target_position.x = flock.size_x * flock.scale_x - target_position.x;
    
    if (flock.flip_v != 0.0)
      target_position.y = flock.size_y * flock.scale_y - target_position.y;
    ...
    float force_amount = 300 * length(direction) * length(direction);
    force_amount = min(force_amount, 500 * 50);
    vec2 force = normalize(direction) * force_amount;

    uint alt_seed1 = hash(gl_GlobalInvocationID.x + uint(1));

    vec2 wind = vec2(-force.y, force.x);
    wind = normalize(wind);
    wind *= sin(rand_from_seed(alt_seed1) * 300 + length(current_position) / 50) * min(length(velocity) * length(velocity) * 0.2, 500 * 10);
    ...
    current_position += velocity * params.delta;
